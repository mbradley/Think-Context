...
var HttpPouch = function(opts, callback) {
  ...
  api.query = function(fun, opts, callback) {
	...
	params.push('key=' + JSON.stringify(opts.key));
    }
    ...
    if (typeof fun === 'string') {
      ...
      ajax({
        auth: host.auth,
        type:'GET',
          url: genUrl(host, '_design/' + parts[0] + '/_view/' + parts[1] + encodeURI(params)),
      }, callback);
      return;
    }
...
//      docInfo.data._rev = docInfo.metadata.rev;
	docInfo.data._rev = winningRev(docInfo.metadata.rev_tree[0].pos, docInfo.metadata.rev_tree[0].ids);
